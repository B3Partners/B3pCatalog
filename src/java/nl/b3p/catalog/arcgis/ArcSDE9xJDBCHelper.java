...
public class ArcSDE9xJDBCHelper extends ArcSDEJDBCHelper {
    ...
    public List<Dir> getFeatureDatasets(final String currentPath) throws NamingException, SQLException {
        ...
        try {
            return new QueryRunner().query(c, "select databasename, owner, name from " + getTableName(TABLE_FEATUREDATASET), new ResultSetHandler<List<Dir>>() {
    ...
    public void saveMetadata(ArcSDEJDBCDataset dataset, String metadata) throws Exception { 
        ...
        try {
            ...
            if(rowsAffected == 0) {
                ...
                ps = c.prepareStatement("insert into " + getTableName(TABLE_USERMETADATA)+ " (id, databasename, owner, name, datasettype, xml) values(?,?,?,?,?,?)");
                ps.setObject(1, id);
    ...
    private Integer determineDatasetType(Connection c, ArcSDEJDBCDataset dataset) throws Exception {
        ...
        Object isDataset = runner.query(
                c, 
                "select 1 from " + getTableName(TABLE_FEATUREDATASET) + " where owner = ? and name = ?" + databaseNameSQL(dataset), 
                new ScalarHandler(), 
                datasetParams);
        ...
        Integer featureType = (Integer)runner.query(c,
                "select fc.FeatureType from " + getTableName(TABLE_OBJECTCLASSES) + " oc " +
        ...
        if(featureType == esriFeatureType.esriFTSimple) {
            ...
            Integer isRasterDataset = (Integer)runner.query(c,
                    "select rc.isRasterDataset from " + getTableName(TABLE_RASTERCATALOGS) + " rc " +
