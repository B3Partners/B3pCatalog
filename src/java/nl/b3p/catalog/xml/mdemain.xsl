...
<xsl:stylesheet
        ...
        exclude-result-prefixes="xlink gmd gfc gmx gco gml b3p pbl dc srv"
        >
  
	<!-- These parameters must be set by the browser -->
    ...
    <xsl:template name="serviceItems">
        <xsl:for-each select="/gmd:MD_Metadata | /*/gmd:MD_Metadata">
            ...
            <!-- ISO 394 Dataset referentie datum MD_Metadata.identificationInfo>MD_DataIdentification.citation>CI_Citation.date>CI_Date.date-->
            <!-- ISO 365 MD_Metadata.identificationInfo>MD_DataIdentification.citation>CI_Citation.identifier>MD_Identifier.code -->
            <xsl:apply-templates select="gmd:identificationInfo/srv:SV_ServiceIdentification/gmd:citation//gmd:identifier/gmd:MD_Identifier/gmd:code"/>            <!-- ISO 395 Creatie-, publicatie-, of wijzigingsdatum MD_Metadata.identificationInfo>MD_DataIdentification.citation>CI_Citation.date>CI_Date.dateType Codelijst: CI_DateTypeCode (B.5.2)-->
    ...
    <xsl:template name="add-comments-content">
        ...
        <textarea id="ui-mde-comment-textarea" cols="80" rows="10"></textarea>
        <div class="ui-mde-margin-top-bottom-area">
    ...
    <!-- ISO 9 Metadata datum MD_Metadata.dateStamp -->
    <xsl:template match="gmd:dateStamp/*"> <!-- gmd:dateStamp/gco:Date or gmd:dateStamp/gco:DateTime -->
        <xsl:call-template name="element">
    ...
    <xsl:template match="gmd:identificationInfo/*/gmd:descriptiveKeywords">
        <div class="ui-mde-section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Trefwoorden uit thesaurus <xsl:call-template name="position"/></xsl:with-param>
                <xsl:with-param name="repeatable" select="true()"/>
            </xsl:call-template>
            <div class="ui-mde-section-content">
    ...
    <xsl:template match="gmd:identificationInfo/*/gmd:descriptiveKeywords[1]/gmd:MD_Keywords/gmd:keyword/gco:CharacterString">
        <xsl:call-template name="element"><!-- count(preceding-sibling::gmd:descriptiveKeywords) = 0 -->
    ...
    <xsl:template match="gmd:identificationInfo/*/gmd:descriptiveKeywords/gmd:MD_Keywords/gmd:thesaurusName/gmd:CI_Citation/gmd:date">
        <div class="ui-mde-section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Thesaurus datum <xsl:call-template name="position"/></xsl:with-param>
                <xsl:with-param name="repeatable" select="true()"/>
            </xsl:call-template>
            <div class="ui-mde-section-content">
    ...
    <xsl:template name="resourceConstraints">
        <xsl:param name="resourceConstraints"></xsl:param>
        <div class="ui-mde-section">
            <xsl:call-template name="section-title">
    ...
    <xsl:template match="gmd:resourceConstraints/gmd:MD_LegalConstraints">
        <div class="ui-mde-section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Juridische beperkingen <xsl:call-template name="position"/></xsl:with-param>
            </xsl:call-template>
            <div class="ui-mde-section-content">
    ...
    <xsl:template match="gmd:dataQualityInfo/gmd:DQ_DataQuality/gmd:lineage/gmd:LI_Lineage/gmd:source">
        <div class="ui-mde-section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Bron <xsl:call-template name="position"/></xsl:with-param>
                <xsl:with-param name="repeatable" select="true()"/>
            </xsl:call-template>
            <div class="ui-mde-section-content">
    ...
    <xsl:template match="gmd:dataQualityInfo/gmd:DQ_DataQuality/gmd:lineage/gmd:LI_Lineage/gmd:source/gmd:LI_Source/gmd:sourceStep">
        <div class="ui-mde-section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Inwinningsstap <xsl:call-template name="position"/></xsl:with-param>
                <xsl:with-param name="repeatable" select="true()"/>
            </xsl:call-template>
            <div class="ui-mde-section-content">
    ...
    <xsl:template match="gmd:dataQualityInfo/gmd:DQ_DataQuality/gmd:lineage/gmd:LI_Lineage/gmd:source/gmd:LI_Source/gmd:sourceStep/gmd:LI_ProcessStep/gmd:processor">
        <div class="ui-mde-section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Inwinnende organisatie <xsl:call-template name="position"/></xsl:with-param>
                <xsl:with-param name="repeatable" select="true()"/>
            </xsl:call-template>
            <div class="ui-mde-section-content">
    ...
    <xsl:template match="gmd:referenceSystemInfo">
        <div class="ui-mde-section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Referentiesysteem <xsl:call-template name="position"/></xsl:with-param>
                <xsl:with-param name="repeatable" select="true()"/>
            </xsl:call-template>
            <div class="ui-mde-section-content">
    ...
    <!-- ISO 351 Temporele dekking - BeginDatum/einddatum MD_Metadata.identificationInfo>MD_DataIdentification.extent>EX_Extent.temporalElement>EX_TemporalExtent.extent TM_Primitive(B.4.5) -->
    <xsl:template match="gmd:EX_Extent/gmd:temporalElement"><!--name="temporeleDekking"-->
        <div class="ui-mde-section">
            <xsl:call-template name="section-title">
    ...
    <xsl:template match="gmd:MD_Metadata/gmd:contact">
        <div class="ui-mde-section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Verantwoordelijke organisatie metadata <xsl:call-template name="position"/></xsl:with-param>
                <xsl:with-param name="repeatable" select="true()"/>
            </xsl:call-template>
            <div class="ui-mde-section-content">
                <xsl:apply-templates select="gmd:CI_ResponsibleParty">
    ...
    <xsl:template match="gmd:identificationInfo/*/gmd:pointOfContact">
        <div class="ui-mde-section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Verantwoordelijke organisatie bron <xsl:call-template name="position"/></xsl:with-param>
                <xsl:with-param name="repeatable" select="true()"/>
            </xsl:call-template>
            <div class="ui-mde-section-content">
                <xsl:apply-templates select="gmd:CI_ResponsibleParty">
    ...
    <xsl:template match="gmd:distributionInfo//gmd:distributor">
        <div class="ui-mde-section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Distribuerende organisatie <xsl:call-template name="position"/></xsl:with-param>
                <xsl:with-param name="repeatable" select="true()"/>
            </xsl:call-template>
            <div class="ui-mde-section-content">
    ...
    <xsl:template match="gmd:citation/*/gmd:date">
        <div class="ui-mde-section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Datum van de bron <xsl:call-template name="position"/></xsl:with-param>
                <xsl:with-param name="repeatable" select="true()"/>
            </xsl:call-template>
            <div class="ui-mde-section-content">
                <xsl:apply-templates select="gmd:CI_Date">
    ...
    <xsl:template match="gmd:distributionInfo/gmd:MD_Distribution/gmd:distributionFormat">
        <div class="ui-mde-section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Distributie formaat <xsl:call-template name="position"/></xsl:with-param>
                <xsl:with-param name="repeatable" select="true()"/>
            </xsl:call-template>
            <div class="ui-mde-section-content">
    ...
    <xsl:template match="gmd:distributionInfo/gmd:MD_Distribution/gmd:transferOptions/gmd:MD_DigitalTransferOptions/gmd:onLine">
        <div class="ui-mde-section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">URL dataset <xsl:call-template name="position"/></xsl:with-param>
                <xsl:with-param name="repeatable" select="true()"/>
            </xsl:call-template>
            <div class="ui-mde-section-content">
                ...
                <xsl:choose>
                    <xsl:when test="gmd:CI_OnlineResource/gmd:protocol/gco:CharacterString"> <!-- This is the correct one: -->
                        <xsl:apply-templates select="gmd:CI_OnlineResource/gmd:protocol/gco:CharacterString"/>
                    </xsl:when>
                    <xsl:otherwise> <!-- If non-standard gmd:protocol/gmd:SV_ServiceType exist we use it. -->
    ...
    <xsl:template match="gmd:identificationInfo/srv:SV_ServiceIdentification/srv:containsOperations/srv:SV_OperationMetadata">
        <div class="ui-mde-section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Service operaties <xsl:call-template name="position"/></xsl:with-param>
                <xsl:with-param name="repeatable" select="true()"/>
            </xsl:call-template>
            <div class="ui-mde-section-content">
    ...
    <xsl:template match="gmd:identificationInfo/srv:SV_ServiceIdentification/srv:operatesOn">
        <div class="ui-mde-section"> 
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Dataset <xsl:call-template name="position"/></xsl:with-param>
                <xsl:with-param name="repeatable" select="true()"/>
                <xsl:with-param name="repeatable-path" select="."/>
            </xsl:call-template>
            <div class="ui-mde-section-content">  
                <xsl:call-template name="element">
                    ...
                    <xsl:with-param name="path" select="./@uuidref"></xsl:with-param>
                    <xsl:with-param name="attrName" select="'uuidref'"></xsl:with-param>
                    <xsl:with-param name="optionality" select="'conditional'"/>
                ...
                <xsl:call-template name="element">
                    ...
                    <xsl:with-param name="path" select="./@xlink:href" ></xsl:with-param>
                    <xsl:with-param name="attrName" select="'xlink:href'"></xsl:with-param>
                    <xsl:with-param name="optionality" select="'conditional'"/>
                    <xsl:with-param name="help-text" select="'ISO 19110 operatesOn'"/>
                    <xsl:with-param name="help-link">http://wiki.geonovum.nl/index.php/2.5.27_De_link_naar_de_metadata_van_de_dataset_en_dataset_series_vanuit_de_service</xsl:with-param>
    ...
    <xsl:template match="gmd:identificationInfo/srv:SV_ServiceIdentification/srv:coupledResource">
        <div class="ui-mde-section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Gekoppelde dataset <xsl:call-template name="position"/></xsl:with-param>
                <xsl:with-param name="repeatable" select="true()"/>
                <xsl:with-param name="repeatable-path" select="."/>
            </xsl:call-template>
            <div class="ui-mde-section-content">
    ...
    <xsl:template match="gmd:aggregationInfo">
        <div class="ui-mde-section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Gerelateerde dataset <xsl:call-template name="position"/></xsl:with-param>
                <xsl:with-param name="repeatable" select="true()"/>
            </xsl:call-template>
            <div class="ui-mde-section-content">
    ...
    <xsl:template match="gmd:aggregateDataSetName/gmd:CI_Citation/gmd:date">
        <div class="ui-mde-section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Datum van de gerelateerde dataset <xsl:call-template name="position"/></xsl:with-param>
                <xsl:with-param name="repeatable" select="true()"/>
            </xsl:call-template>
            <div class="ui-mde-section-content">
                <xsl:apply-templates select="gmd:CI_Date">
    ...
    <xsl:template match="gfc:featureType">
        <div class="ui-mde-section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Feature type <xsl:call-template name="position"/></xsl:with-param>
                <xsl:with-param name="repeatable" select="true()"/>
            </xsl:call-template>
            <div class="ui-mde-section-content">
    ...
    <xsl:template name="element">
        ...
        <xsl:param name="optionality">optional</xsl:param><!-- 'conditional' of 'mandatory' of 'optional' of leeg (= optional). Dit is feedback voor de user. -->
        <xsl:param name="repeatable" select="false()"/>
        <xsl:param name="repeatable-path" select="."/> <!-- For 19115/19139 fields, the parent (..) should be passed here. This is not the case for, for example, DC -->
        <xsl:param name="readonly" select="$globalReadonly"/>
        <xsl:param name="type">normal</xsl:param><!-- speciaal type: normal, rich-text of image-url (dit zijn allemaal gco:CharacterString's; uit het data-type is dus niets af te leiden) -->
        ...
        <div class="{$element-class}">
            ...
            <xsl:element name="div">
                <xsl:attribute name="class">
                    ...
                    <xsl:if test="$bold-value = true()">_bold</xsl:if>
                </xsl:attribute>
                ...
                <!-- custom attributes -->
                <xsl:attribute name="ui-mde-fullpath">
                    <xsl:call-template name="full-path">
    ...
    <xsl:template name="repeatable-menu">
        ...
        <span class="menu-wrapper">
            <xsl:element name="img">
                ...
                <xsl:attribute name="title">
                    <xsl:value-of select="$MENU_TOOLTIP"/>
                </xsl:attribute>
...
<img src="data:image/gif;base64,R0lGODlhDwAPAKECAAAAzMzM/////wAAACwAAAAADwAPAAACIISPeQHsrZ5ModrLlN48CXF8m2iQ3YmmKqVlRtW4MLwWACH+H09wdGltaXplZCBieSBVbGVhZCBTbWFydFNhdmVyIQAAOw==" alt="base64 encoded image" width="150" height="150"/>
    ...
    <xsl:template name="wikiHelpPopupLink">
        <xsl:if test="$globalReadonly = false()">
